...
class DataTrainingArguments:
    ...
    do_binary_classification: Optional[bool] = field(
        default=False, metadata={"help": "Set to true, if binary classification should be performed, false otherwise."}
    )
...
def main():
    ...
    def preprocess_function(examples):
        if data_args.text_column_names is not None:
            ...
            assert len(text_column_names) == 2
            examples["article"] = examples[text_column_names[0]]
            examples["sentence"] = examples[text_column_names[1]]

        result = tokenizer(examples["article"], examples["sentence"], padding=padding, max_length=max_seq_length,
                           truncation=True)
    ...
    logger.info(
        "Using f1, recall, precision and balanced accuracy as classification score, currently only this metrics are "
        "supported for Absinth.")
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        results = {}
        f1_all_results = f1_metric.compute(predictions=preds, references=p.label_ids, average=None)['f1']
        if data_args.do_binary_classification:
            class_labels = ["F1_Hallucination", "F1_Faithful"]
        else:
            class_labels = ["F1_Extrinsic", "F1_Faithful", "F1_Intrinsic"]
        f1_all_results = dict(zip(class_labels, f1_all_results))
        results.update(f1_all_results)
        results["Bacc"] = balanced_accuracy_score(y_true=p.label_ids, y_pred=preds)
        results.update(f1_metric.compute(predictions=preds, references=p.label_ids, average='macro'))
        results.update(recall_metric.compute(predictions=preds, references=p.label_ids, average='macro'))
        results.update(precision_metric.compute(predictions=preds, references=p.label_ids, average='macro'))
        return results
